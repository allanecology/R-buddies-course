---
title: "Beginner -  Data wrangling in R"
author: "N. Schenk, C.Daniel"
date: "2022-11-09"
...
vec > 2                 # show for each element of the vector if the statement is true or false
...
species<- c("taraxacum","soldanella", "holcus", "veronica")

grepl("soldanella",species)
species=="soldanella"


```

**Exercise 1:** Try to do the same with a character string instead of a numeric string
<br>
...
- from `R base` : `rbind()` and `cbind()` to combine two datasets (without using a common column), or `merge()` to combine two datasets by a common column 

<br>

(For more info, see advanced data wrangling tutorial)
...
**Exercise 2:** Try to remove two species using conditional operators
...
- Change a numeric variable of the "iris" dataset into a factor, then into a character string without changing the values themselves.